...
candidates (raw extraction store, denormalized source metadata)
  -> source_registry (one row per item+source+run combination)
...
Host/domain fields are provenance metadata only; authority/mutation routing is ID-slot based (`item_field_state.id`, `component_values.id`, `list_values.id`, `enum_lists.id`, `key_review_state.id`).
